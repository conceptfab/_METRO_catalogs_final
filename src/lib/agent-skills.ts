...
import { DNS_AID_SKILL_MD } from '@/lib/dnsaid';
...
const DNS_AID_SKILL = {
  ...
  type: 'skill-md',
  description:
    'DNS for AI Discovery (DNS-AID) descriptor — lists the SVCB/HTTPS records to publish under _agents.<domain> for this site.',
  url: '/.well-known/agent-skills/dns-aid/SKILL.md',
} as const;
...
export function buildAgentSkillsIndex() {
  return {
    ...
    skills: [
      ...
      {
        ...DNS_AID_SKILL,
        digest: sha256Digest(DNS_AID_SKILL_MD),
      },
    ],
  };
}
