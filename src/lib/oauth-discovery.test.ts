...
describe('buildOAuthDiscoveryMetadata', () => {
  ...
  it('exposes registration_endpoint and revocation_endpoint', () => {
    const meta = buildOAuthDiscoveryMetadata();
    expect(meta.registration_endpoint).toBe(
      'https://test.metro.example/oauth/register',
    );
    expect(meta.revocation_endpoint).toBe(
      'https://test.metro.example/oauth/revoke',
    );
  });

  it('exposes agent_auth block in the canonical auth.md shape', () => {
    const meta = buildOAuthDiscoveryMetadata();
    expect(meta.agent_auth).toBeDefined();
    expect(meta.agent_auth.skill).toBe('https://test.metro.example/auth.md');
    expect(meta.agent_auth.register_uri).toBe(
      'https://test.metro.example/oauth/register',
    );
    expect(meta.agent_auth.revocation_uri).toBe(
      'https://test.metro.example/oauth/revoke',
    );
...
describe('buildProtectedResourceMetadata', () => {
  ...
  it('points to auth.md and registration/revocation endpoints', () => {
    const meta = buildProtectedResourceMetadata();
    expect(meta.auth_md_uri).toBe('https://test.metro.example/auth.md');
    expect(meta.registration_endpoint).toBe(
      'https://test.metro.example/oauth/register',
    );
    expect(meta.revocation_endpoint).toBe(
