...
function useHeroQXViewModel(data: HeroData) {
  ...
  const isCoverVisibleRef = useRef(true);
  const subscribeCoverVisibility = (onChange: () => void) => {
    const cover = document.getElementById('cover');
    if (!cover) return () => {};
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (isCoverVisibleRef.current !== entry.isIntersecting) {
          isCoverVisibleRef.current = entry.isIntersecting;
          onChange();
        }
      },
      { threshold: 0.25 },
  ...
  const isCoverVisible = useSyncExternalStore(
    subscribeCoverVisibility,
    () => isCoverVisibleRef.current,
    () => true,
  );
