...
const MaterialsQX = ({ data }: MaterialsSectionProps) => {
  ...
  const frameOptions = (() => {
    ...
    return applyOptionDescriptions(
      [...ordered, ...leftovers],
      FRAME_COLOR_NAMES,
    );
  })();
  ...
  const configuratorAlt =
    selectedFrame && selectedDesktop
      ...
      : `${data.title} preview`;
  const previewLayerClass =
    'materials-configurator-preview-layer absolute inset-0 h-full w-full object-contain';
    ...
    >
      ...
      >
        ...
        <m.div
          ...
          className="materials-configurator-preview-host mt-10 aspect-square w-full lg:absolute lg:bottom-0 lg:left-0 lg:mt-0 lg:aspect-auto lg:h-[715px] lg:w-[687px]"
        >
          {hasConfigurator && selectedFrame && selectedDesktop ? (
            <figure
              className="materials-configurator-preview relative h-full w-full"
              role="img"
