...
/**
 * Print-only Arrangements / "Sample arrangements" section for MRC1000.
 *
 * Renders the same `placeholders` from arrangements/content.json that the
 * on-screen ArrangementsMRC1000 uses — but paged for A4 landscape print. Reuses
 ...
 */
export default function ArrangementsPrintMRC1000({ data }: Props) {
  ...
  return (
    <>
      {pages.map((pageItems, pageIndex) => (
        <div
          key={`arrangements-page-${pageItems[0]?.image ?? pageItems[0]?.label}`}
